...
export function LmInstagramList({ content }: LmInstagramListProps) {
  ...
  const { data } = useSWR(() => inView ? `${security}://www.instagram.com/${username}/?__a=1` : null, fetcher)
  let posts: InstagramMappedProps[] = data?.graphql?.user?.edge_owner_to_timeline_media?.edges?.filter(
    (i: { node: EdgeProps }) => {
  ...
  return (
    <div ref={refIntersectionObserver}
      ...
      <GridList className={gridClasses.gridList}
        ...
        {(posts || []).map((item) => (
          <GridListTile key={item.shortcode}
                        component={'a'}
                        classes={{
                          tile: clsx({
                            [classesShadow[content.shadow_effect || '']]: !!content.shadow_effect
                          })
                        }}
